...
$versionUrl = "https://googlechromelabs.github.io/chrome-for-testing/last-known-good-versions.json"
$lastKnownGoodVersions = Invoke-RestMethod $versionUrl
$currentStable = $lastKnownGoodVersions.channels.stable.version

if(Test-Path $chromeDriverVersionPath) {
	$chromeDriverVersion = Get-Content -Path $chromeDriverVersionPath
}

if($chromeDriverVersion -eq $currentStable) {
	Write-Host "ChromeDriver is up to date at $chromeDriverVersion. Skipping download."
	return
}

New-Item -Path $chromeDriverVersionPath -ItemType "file" -Value "$currentStable" -Force

$downloadUrl = "https://edgedl.me.gvt1.com/edgedl/chrome/chrome-for-testing/$currentStable/$os/chromedriver-$os.zip"
